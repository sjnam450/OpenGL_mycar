...
void Robot::moveHeadZ(float speed){
    ...
    headAngleZ = std::asinf(m_vHeadView.v[1]/m_vHeadView.length())* _180_OVER_PI; //Update rotation angle for display
}
...
{   
    ...
    headAngleZ = std::asinf(m_vHeadView.v[1]/m_vHeadView.length())* _180_OVER_PI; //Calculate head angle
}
